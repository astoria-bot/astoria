...
from bot.utils.metadata import MemberID
...
class Moderation(commands.Cog):
    ...
    async def role(self, ctx, member: discord.Member, role: discord.Role):
        ...
        if role in member.roles:  # User already has the role; remove it
            ...
            await ctx.send(f"The {role.name} role was removed from {member.name}.")
        else:  # User does not have role, assign it to them
            await member.add_roles(role)
            await ctx.send(f"The {role.name} role has been assigned to {member.name}.")

    @commands.has_permissions(manage_messages=True, read_message_history=True)
    @commands.command()
    async def clear(self, ctx, msgs: int = 1):
        """Deletes the specified amount of messages.
        Delete 1 message by default."""
        if msgs > 100:
